...
DomainParticipant* Communicator::CreateParticipant(long domain) {
...
DomainParticipant* Communicator::CreateParticipant() {
  _participant = TheParticipantFactory->create_participant(
...
DomainParticipant* Communicator::CreateParticipant(
  ...
  _participant = TheParticipantFactory->create_participant_with_profile(
      domain, participantQosLibrary.c_str(), participantQosProfile.c_str(),
...
Publisher* Communicator::CreatePublisher() {
  if (GetParticipant() == NULL) {
    std::stringstream errss;
    errss << "DomainParticipant NULL - communicator not properly "
          << "initialized";
    throw errss.str();
  }
  ...
  //
  _pub = GetParticipant()->create_publisher(PUBLISHER_QOS_DEFAULT, NULL,
                                            STATUS_MASK_NONE);
...
Publisher* Communicator::CreatePublisher(const std::string& qosLibrary,
  ...
  _pub = GetParticipant()->create_publisher_with_profile(
      qosLibrary.c_str(), qosProfile.c_str(), NULL, STATUS_MASK_NONE);
...
Subscriber* Communicator::CreateSubscriber() {
  ...
  //  in the Communicator class because one Subscriber can be used to
  //  create multiple DDS DataReaders.
  //
  _sub = GetParticipant()->create_subscriber(SUBSCRIBER_QOS_DEFAULT, NULL,
                                             STATUS_MASK_NONE);
...
Subscriber* Communicator::CreateSubscriber(const std::string& qosLibrary,
                                              const std::string& qosProfile) {
  if (GetParticipant() == NULL) {
    std::stringstream errss;
    errss << "DomainParticipant NULL - communicator not properly "
